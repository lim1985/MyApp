...
import { SortCustomGroupUsers } from '@/api/manage'
  ...
  props:{
   ...
   updated() {
    ...
    let arr=this.list.map((item,index,arr)=>{
        ...
        return {ID:item.ResGroupID,GroupID:item.CustomGroupID,UserPhoneID:item.ID,OrderID:_length-index}
    })
     ...
     async handleUpdataOk()
      {
          ...
          SortCustomGroupUsers(_data).then(res=>{
              console.log(res)
              if(res.msg=='SortSuccess')
              {
                setTimeout(() => {

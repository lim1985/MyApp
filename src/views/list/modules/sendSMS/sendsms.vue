    ...
    >
      <a-spin :spinning="confirmLoading">
        <a-row type="flex" justify="start">
          <a-col :offset="1" :xs="12" :sm="12" :md="16" :lg="16" :xl="16">
            <p class="height-100">
              <strong style="color: red">温馨提示</strong
              >：批量发送可以从右边列表直接勾选联系人。
            </p>
          </a-col>
          <a-col :offset="1" :xs="10" :sm="10" :md="6" :lg="6" :xl="6">
            <p class="height-100">
              <a-input
                style="width: 180px"
                placeholder="填写联系人的手机或姓名"
...
import { SendSMS, SmsAddrecord, SmsSucceedcount, UpdateDepSmsCount, newChinaCMCCSendSMS, GetUserInformationByUserNameLIke, GetUserInfoByTelOrPhoneNum } from '@/api/manage' //CMCCSendSMS
...
export default {
  ...
  methods: {
    async addphone () {
      ...
      let body = {}
      let flag = false;
      if (this.phoneValue.flag == 0) {
        //  GetUserInfoByTelOrPhoneNum({ tel: num })
        res = await GetUserInfoByTelOrPhoneNum({ tel: this.phoneValue.value })

        if (res.code == -1) {
          this.$message.error('未搜索到该联系人');
        }
        body.DPname = res.res['ResferecDep.Abbreviation']
        body.ID = res.res.ID
        body.Phone = res.res.cellphone
        body.UJOB = res.res.UJOB
        body.checked = false
        body.username = res.res.UserName
      } else {
        res = await GetUserInformationByUserNameLIke({ data: this.phoneValue.value, parameter: this.parameter })

        if (res.code == -1) {
          this.$message.error('未搜索到该联系人');
        }
        body.DPname = res.res.data[0].Abbreviation
        body.ID = res.res.data[0].ID
        body.Phone = res.res.data[0].cellphone
        body.UJOB = res.res.data[0].UJOB
        body.checked = false
        body.username = res.res.data[0].UserName
      }
      ...
      // ID: 1184
      // OrderID: null
      // Permission_Key: "QXCXT"
      // QQ: null
      // ResferecDep.LIM_ResferenceAndDep.DepID: 167
      // ResferecDep.LIM_ResferenceAndDep.ID: 164
      // ResferecDep.LIM_ResferenceAndDep.OrderID: 5
      // ResferecDep.LIM_ResferenceAndDep.UserPhoneID: 1184
      // ResferecDep.LIM_ResferenceAndDep.status: -1
      // Sex: 1
      // Tel: "5396272"
      // UJOB: "主任"
      // UserName: "张海波"
      // cellphone: "13807399838"
      // status: 9
      // body.DPname = res.res['ResferecDep.Abbreviation']
      // body.ID = res.res.ID
      // body.Phone = res.res.cellphone
      ...
      console.log(flag)
      if (!flag) {
        this.Pupuarr.unshift(body);
      }
      else {
        this.$message.error('请不要重复添加联系人')
      }


    },
    checkphoneNum: _.debounce(async function (e) {
      ...
      //0 为手机号 1为姓名
      // const { value } = e.target
      //GetUserInfoByTelOrPhoneNum
      //GetUserInformationByUserNameLIke
      let reg2 = /^[\u0391-\uFFE5]{0,4}$/;
      ...
      // that.flag = reg.test(value) ? 0 : -1
      // that.flag = reg2.test(value) ? 1 : -1
      console.log(that.flag)
        ;//控制按钮 可用或不可用
      that.phoneValue = Object.assign({ value: value }, { flag: that.flag });
    ...
    async CmccSendSMS () {
      ...
      let smsArr = newChinaCMCCSendSMS(params).then(r => {
        ...
        if (_json.rspcod == 'success' && _json.success == true) {
          for (let x in this.Pupuarr) {
            if (!this.Pupuarr[x].checked == false) {            // params.mobiles
